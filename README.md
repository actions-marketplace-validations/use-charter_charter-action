...
> This repository is a published **mirror**. The source of truth is the Charter monorepo at [`use-charter/charter`](https://github.com/use-charter/charter) under `action/`. Open issues and PRs against the monorepo, not here.

The action is developed in the monorepo under `action/` and published to this standalone `use-charter/charter-action` repository at release. Consumers reference `use-charter/charter-action@v1` (a moving major tag) or a pinned `@v1.0.0`.
